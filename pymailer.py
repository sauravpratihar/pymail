    ...
    def send(self, retry_count=0, recipient_list=None):
        ...
        for recipient_data in recipient_list:
            ...
            if recipient_data.get('name'):
                recipient = "%s <%s>" % (recipient_data.get('name'), recipient_data.get('email')) #with name
                # recipient = "<%s>" % (recipient_data.get('email')) #without name only email
            ...
            check = smtp_server.login(config.EMAIL,config.PASS)
            try:
...
def main(sys_args):
    ...
    try:
        ...
        print "Not enough argumants supplied. PyMailer requests 1 option (-t for test mail and -s for normal mail) and 3 arguments: ./pymailer -s html_file csv_file subject"
        sys.exit()
